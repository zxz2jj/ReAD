...
import os
import time
from cifar10_model import train_models
...
if __name__ == "__main__":
    ...
    for category_of_train in range(2, 10):
        print('*******************************Train Category: 0-{}, OOD Category: {}-9.******************************'.
              format(category_of_train - 1, category_of_train))
        # train mnist model. If model is existed, it will show the information of model
        gv.category_classified_of_model = category_of_train
        gv.model_path = 'models/cifar_model_0-{}.h5'.format(category_of_train-1)
        show_model = True
        if show_model:
            train_models(category_of_train)

        filepath_of_neural_value_statistic = \
            gv.detector_path + 'train_picture_neural_value_statistic_{}.pkl'.format(category_of_train - 1)
        filepath_of_k_means_centers = gv.detector_path + 'k_means_centers_{}.pkl'.format(category_of_train - 1)
        filepath_of_distance = \
            gv.detector_path + 'distance_of_train_data_between_abstraction_and_center_{}.pkl'.format(category_of_train-1)

        train_picture_neural_value_statistic = None
        k_means_centers = None
        distance_of_train_data_between_abstraction_and_center = None
        if os.path.exists(filepath_of_neural_value_statistic) and os.path.exists(filepath_of_k_means_centers) \
                and os.path.exists(filepath_of_distance):
            print("\nOOD Detector is existed!")
            file1 = open(filepath_of_neural_value_statistic, 'rb')
            file2 = open(filepath_of_k_means_centers, 'rb')
            file3 = open(filepath_of_distance, 'rb')
            ...
            # ********************** Train Detector **************************** #
            print('\n********************** Train Detector ****************************')
            train_picture_classified = classify_train_data(category_of_train)
            ...
            file1 = open(filepath_of_neural_value_statistic, 'wb')
            ...
            train_picture_combination_abstraction = encode_combination_abstraction(
                ...
                train_neural_value_statistic=train_picture_neural_value_statistic)
            train_picture_combination_abstraction_concatenated = \
                concatenate_data_between_layers(train_picture_combination_abstraction)

            # visualization will need some minutes.
            # If you want to show the visualization, please run the following codes.
            ...
            k_means_centers = k_means(data=train_picture_combination_abstraction_concatenated,
                                      category_number=gv.category_classified_of_model)
            file2 = open(filepath_of_k_means_centers, 'wb')
            ...
            file3 = open(filepath_of_distance, 'wb')
            pickle.dump(distance_of_train_data_between_abstraction_and_center, file3)

        # ********************** Evaluate Detector **************************** #
        print('\n********************** Evaluate Detector ****************************')
        test_picture_classified = classify_test_data(category_of_train)
        ...
        print('\n************Evaluating*************')
        print('In-Distribution Data: MNIST test data category 0-{},\n '
              'Out-of-Distribution Data: MNIST test data category {}-9.'.format(category_of_train-1, category_of_train))
        ood_data, number_of_ood = load_ood_data(category_of_train)
        print('\nGet neural value...')
        t1 = time.time()
        ood_picture_neural_value = get_neural_value(ood_data, gv.fully_connected_layers)
        print('\nEncoding combination abstraction...')
