...
from load_data import load_cifar10_of_given_category
...
class CifarModel(object):
    ...
    def create_model(self):
        ...
        model.add(tf.keras.layers.Convolution2D(40, (5, 5), strides=(1, 1), input_shape=self.train_data.shape[1:]))
        model.add(tf.keras.layers.BatchNormalization())
        model.add(tf.keras.layers.Activation('relu'))
        model.add(tf.keras.layers.MaxPooling2D(pool_size=(2, 2)))
        # model.add(Dropout(0.25))

        model.add(tf.keras.layers.Convolution2D(20, (5, 5), strides=(1, 1)))
        model.add(tf.keras.layers.BatchNormalization())
        model.add(tf.keras.layers.Activation('relu'))
        model.add(tf.keras.layers.MaxPooling2D(pool_size=(2, 2)))
        # model.add(Dropout(0.25))
...
def train_models(category_of_train):
    ...
    gv.x_train, gv.y_train_category, gv.y_train_one_hot,\
      ...
      load_cifar10_of_given_category(category_of_train)

    mnist_model = CifarModel(train_class_number=gv.category_classified_of_model,
...
if __name__ == "__main__":
    for category_number in range(2, 10):
        ...
        gv.model_path = 'models/cifar_model_0-{}.h5'.format(category_number-1)
